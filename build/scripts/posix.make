...
.SILENT : all libs tools csprng-$(version).a csprng-$(version).so $(encoders) $(decoders) install purge
all : libs tools
libs : csprng-$(version).a csprng-$(version).so
tools : get-cryptorandom-bytes
...
# 	cp ./documentation/credits.csv /usr/local/share/csprng
# 	cp ./documentation/releases.csv /usr/local/share/csprng
# 
purge :
	-rm -f /usr/local/lib/csprng.so
...
get-cryptobytes : csprng-$(version).a
	echo $(echoflags) "Building the get-cryptobytes tool... \c"
	dmd \
